...
var Rs = class extends ws {
...
}, Q = document.getElementById("messages"), $ = document.getElementById("input"), Hs = document.getElementById("send"), Us = document.getElementById("attach-btn"), Ws = document.getElementById("file-input"), Gs = document.getElementById("attachment-preview-container"), Ks = {
	...
	parameters: {
		...
		properties: {
			approval_id: {
				type: "string",
				description: "Unique ID for this approval; use the intended call ID."
			},
			function_name: {
				type: "string",
			...
			function_arguments: {
				type: "object",
				description: "Arguments you intend to pass to the tool."
			}
		},
		required: ["approval_id", "function_name"]
	}
}, qs = localStorage.getItem("ag-ui-thread-id") ?? crypto.randomUUID();
localStorage.setItem("ag-ui-thread-id", qs);
var Js = [], Ys = [];
Us.addEventListener("click", () => Ws.click()), Ws.addEventListener("change", () => {
	if (Ws.files) {
		for (let e of Array.from(Ws.files)) Ys.push(e);
		Xs(), Ws.value = "";
	}
});
function Xs() {
	Gs.innerHTML = "", Ys.forEach((e, t) => {
...
function $s() {
	for (let e of Js) e.role === "user" && typeof e.content == "string" ? Qs("user", e.content) : e.role === "assistant" && typeof e.content == "string" && e.content.length > 0 && Qs("assistant", e.content);
}
async function ec() {
	let e = await fetch(`/history/${encodeURIComponent(qs)}`);
	if (!e.ok) return;
	...
	for (let e of t) {
		...
		r && (e.role === "user" ? Js.push({
			id: t,
			role: "user",
			content: r
		}) : e.role === "assistant" && Js.push({
			id: t,
			role: "assistant",
			content: r
		}));
...
async function rc(e, t) {
	let n = new Vs({ url: "/agui" }), r = t.querySelector(".loading-wave") ? "" : t.textContent ?? "", i = /* @__PURE__ */ new Map(), a = /* @__PURE__ */ new Map();
		...
		n.run(e).subscribe({
			next: (n) => {
				switch (n.type) {
					...
					case N.RUN_ERROR: {
						let e = n;
						t.textContent = `Code: ${e.code}\n Error: ${e.message}`, t.classList.add("error"), Q.scrollTop = Q.scrollHeight;
						break;
					}
					case N.TOOL_CALL_START: {
						let e = n;
					...
					case N.TOOL_CALL_END: {
						let r = n.toolCallId, o = a.get(r) ?? "", s = i.get(r) ?? "{}";
						if (o !== "request_approval") break;
						let c = JSON.parse(s), l = c.approval_id ?? r, u = {
							toolCallId: r,
							approvalId: l,
							functionName: c.function_name ?? "unknown",
							resolve: async (n) => {
								let i = {
			...
			error: (e) => {
				t.textContent = `Error: ${e}`, Hs.disabled = !1, s(e);
			},
			complete: () => o()
		});
	});
...
async function ic() {
	...
	let r = e + (n.length > 0 ? `\n\n[Attached: ${n.join(", ")}]` : ""), i = Ys.length > 0 ? `${e} [File Attached]` : e;
	Ys = [], Gs.innerHTML = "";
	let a = {
		id: crypto.randomUUID(),
		role: "user",
	...
	try {
		await rc(s, o), o.textContent && o.textContent !== "…" && Js.push({
			id: crypto.randomUUID(),
			role: "assistant",
...
function ac() {
	$.style.height = "auto";
	let e = $.scrollHeight;
	$.style.height = `${e}px`, $.style.overflowY = $.clientHeight < e ? "auto" : "hidden";
}
