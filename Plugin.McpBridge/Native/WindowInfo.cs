...
public class WindowInfo
{
	...
	/// <summary>Gets whether the underlying native window still exists.</summary>
	public Boolean Exists => !this.IsEmpty && Native.Window.IsWindow(this.Handle);
	...
	/// <summary>Enumerates visible top-level windows that have a caption (i.e. the windows a user would see on the taskbar).</summary>
	public static IEnumerable<WindowInfo> GetOpenedWindows()
	{
		...
		Native.Window.EnumWindows((hWnd, lParam) =>
		{
			...
			if(info.IsVisible && !String.IsNullOrEmpty(info.Caption))
				result.Add(info);
			return true;
		}, IntPtr.Zero);
