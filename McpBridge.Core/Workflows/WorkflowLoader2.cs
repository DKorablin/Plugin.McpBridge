...
internal sealed class WorkflowLoader2
{
	...
	public async Task<WorkflowHandle> BuildAsync(IEnumerable<AiProviderDto> providers, AIFunction[] tools, CancellationToken cancellationToken = default)
	{
		...
		return await this.BuildCoreAsync(this.Config, providers.ToArray(), tools, cancellationToken);
	}

	private async Task<WorkflowHandle> BuildCoreAsync(
		WorkflowDto config,
		...
		CancellationToken cancellationToken)
	{
		...
		Boolean isEvaluationCacheEnabled = false;
		...
		foreach(WorkflowNode node in config.Nodes)
		{
			...
			if(node.Kind == NodeKind.Workflow)
			{
				...
				WorkflowHandle subHandle = await this.BuildCoreAsync(subConfig, providers, nodeTools, cancellationToken);
				isEvaluationCacheEnabled |= subHandle.IsEvaluationCacheEnabled;
				AIAgent subAgent = subHandle.Workflow.AsAIAgent(name: node.Name);
				ownedResources.Add(subHandle);
			...
			} else
			{
				...
				isEvaluationCacheEnabled |= provider is NetworkProviderDto { EvaluationCacheEnabled: true };
		...
		return new WorkflowHandle(workflow, config.Name, ownedResources, isEvaluationCacheEnabled);
	}
	...
	private async Task<WorkflowHandle> BuildConditionalGraphAsync(
		WorkflowDto config,
		AiProviderDto[] providers,
		AIFunction[] tools,
		CancellationToken cancellationToken)
